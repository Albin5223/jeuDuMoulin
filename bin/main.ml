      ...
      for j = i + 1 to Array.length players - 1 do
        ...
        (try
           for k = 0 to max_games - 1 do
             if Sys.time () -. t >= float_of_int (k + 1) then failwith "Taking too long";
             (try
             ...
             try
               let result = arena player_j player_i Nine_mens_morris in
               match result.winner.color with
               | White -> incr wins_j
               | Black -> incr wins_i
             with _ -> (* Draw: no-one wins *) ()
        ...
        Format.printf "@[<h>%.1f@;<4 4>%s@;<4 4>%d@;<4 4>%s@;<4 4>%d@]@;"
          ((Sys.time () -. t) *. (1000. /. (2. *. float_of_int max_games)))
          pi wins_i_ratio pj wins_j_ratio;
        if wins_i > wins_j then score.(i) <- score.(i) + 1 else if wins_j > wins_i then score.(j) <- score.(j) + 1
      done
    done;
    Format.close_box ();
    Format.open_vbox 0;
    score |> Array.iteri (fun i score -> Format.printf "@[<h>%s@;<4 4>%d@]@;" (fst players.(i)) score);
    Format.printf "@.";
    Printf.eprintf "\n"
