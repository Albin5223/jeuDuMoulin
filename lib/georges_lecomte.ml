...
let rec check_all_moves gu c (i, j) moves =
    ...
    | x :: xs ->
        ...
        if action_creates_mill gu c move then Some move else check_all_moves gu c (i, j) xs
...
let rec possible_mill_moving game_update color (i, j) =
    ...
    then possible_mill_moving game_update color (i + 1, 0)
...
let rec check_all_fly game_update color (i, j) co =
    ...
    then check_all_fly game_update color (i + 1, 0) co
    else if (i, j) = co
    then check_all_fly game_update color (i, j + 1) co
...
let rec first_possible_placing board (i, j) =
    ...
    then first_possible_placing board (i + 1, 0)
...
let rec first_possible_move gu c (i, j) =
    ...
    then first_possible_move gu c (i + 1, 0)
    else if get_square (get_board gu) (i, j) = Some (Color c)
    then
      let moves = possible_moves_directions gu (i, j) c in
      if moves = [] then first_possible_move gu c (i, j + 1) else Moving ((i, j), List.hd moves)
...
let rec first_possible_fly gu c (i, j) =
    ...
    then first_possible_fly gu c (i + 1, 0)
    else if get_square (get_board gu) (i, j) = Some (Color c)
    then
...
let rec first_possible_remove game_update color (i, j) =
    ...
    then first_possible_remove game_update color (i + 1, 0)
    else if get_square (get_board game_update) (i, j) = Some (Color color)
    then Remove (i, j)
