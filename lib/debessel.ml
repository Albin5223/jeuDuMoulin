...
let check_direction_would_mill_from_position
    (board : board)
    ((i, j) : coordinates)
    (color : color)
    (d : direction_deplacement) : bool =
        ...
        | Placing ->
            ...
            let rec check_mil i j () =
                ...
                then None
                  ...
                  | Some Empty ->
                      if check_would_mill_from_position (get_board game_update) (a, b) player.color
                      then Some (a, b)
                      else check_mil a (b + 1) ()
                  | _ -> check_mil a (b + 1) ()
            in
            let place_coord () =
                    ...
                    then check_mill t
                      ...
                      let rec check_mill_move (possible_moves : direction_deplacement list) : action =
                          ...
                          | d :: t ->
                              ...
                              if check_direction_would_mill_from_position (get_board game_update) dirCoords player.color
                                   d
                              then Moving ((i, j), d)
                              else check_mill_move t
                          | _ -> check_mill t
                      in
                      check_mill_move possible_move
                | _ ->
                    let rec choise_mouv () =
