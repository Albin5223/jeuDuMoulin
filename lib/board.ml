...
(**
  Function to get coordinates of a square from another square and a direction. 
  Return None if there is no square in this direction OR if the first step is not a legit Path regarding the direction
  @param board : the board
  @param ij : the coordinate of the departure square
  @param d : the direction of the square that we want to get
*)
let move_from_direction (board : board) ((i, j) : coordinates) (d : direction_deplacement) : coordinates option =
...
let check_mill_from_position (board : board) ((i, j) : coordinates) (color : color) : bool =
    ...
    | Some (Color c) when c = color ->
        let rec count_from_dir (x, y) d =
            match move_from_direction board (x, y) d with
...
let move_to_direction (game : game_update) ((i, j) : coordinates) (d : direction_deplacement) (color : color) :
    game_update =
    match move_from_direction game.board (i, j) d with
    | Some (a, b) -> (
...
let possible_moves (game : game_update) ((i, j) : coordinates) (player : color) : direction_deplacement list =
    let rec aux (game : game_update) ((i, j) : coordinates) (player : color) (acc : direction_deplacement list) :
        ...
        | [] -> []
        | x :: xs -> (
            let destination = move_from_direction game.board (i, j) x in
