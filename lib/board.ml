...
let getRow (board : board) (i : int) : square list = List.nth board i

(* TODO : Return an option *)

(** Function that return the column "j" of the board *)
let getColumn (board : board) (j : int) : square list = List.fold_right (fun l acc -> [List.nth l j] @ acc) board []
...
let coordinateFromDirection (board : board) ((i, j) : coordinates) (d : directionDeplacement) : coordinates option =
    let rec goTo (board : board) ((x, y) : coordinates) (d : directionDeplacement) : coordinates option =
        ...
        then goTo board cooBis d
          ...
          | _ -> None
    in
    if getSquare board (coordinatesFromDirections d (i, j)) = Some (pathToHaveFromDirection d)
    then goTo board (i, j) d
    else None
...
let placePieceOnBoard (board : board) ((i, j) : coordinates) color : gotMill =
    ...
    let check = checkMillFromPosition board (i, j) color in
    (board, check)
...
let placeStartPiece (game : gameUpdate) ((i, j) : coordinates) (color : color) : gameUpdate =
    ...
    then
      ...
      let updatedPlayer =
          {
            phase = concernedPlayer.phase;
            color = concernedPlayer.color;
            piecePlaced = concernedPlayer.piecePlaced + 1;
            nbPiecesOnBoard = concernedPlayer.nbPiecesOnBoard + 1;
            bag = concernedPlayer.bag @ [(i, j)];
          }
      in
      if color = game.player1.color
      then { board; mill = isMill; player1 = updatedPlayer; player2 = game.player2; gameIsChanged = true }
      else { board; mill = isMill; player1 = game.player1; player2 = updatedPlayer; gameIsChanged = true }
    else { board = game.board; mill = false; player1 = game.player1; player2 = game.player2; gameIsChanged = false }
        ...
        let updatedPlayer =
            {
              phase = concernedPlayer.phase;
              color = concernedPlayer.color;
              piecePlaced = concernedPlayer.piecePlaced;
              nbPiecesOnBoard = concernedPlayer.nbPiecesOnBoard - 1;
              bag = newBag;
            }
        in
      ...
      let newPlayer =
          {
            ...
            color = concernedPlayer.color;
            piecePlaced = concernedPlayer.piecePlaced;
            nbPiecesOnBoard = concernedPlayer.nbPiecesOnBoard;
            bag = newBag;
          }
      in
      if color = game.player1.color
                   ...
                   (fun line ->
                     if List.exists
                          (fun x ->
                            ...
                            | _ -> false)
                          line
                     then List.map reverseDiagonal line
                     else line)
                   newBoard)
...
let rec test3SquaresRow
    ...
    then
      test3SquaresRow board width height (i - 1) j (nNodes + 1)
        nbSquares (*if we see a node then we increment the nNodes for the rest of the board*)
...
let rec auxInitBoard (width : int) (height : int) (i : int) (nbSquares : int) (diagonal : bool) (acc : board) : board =
    ...
    then acc (*base case of the function*)
      ...
      let rec squareCoordinates (w : int) (h : int) (start : int) (nbSquares : int) (acc2 : coordinates list) :
          coordinates list =
          if nbSquares <= 0
          then acc2
          ...
          then acc2 @ [(start, start)]
            ...
            squareCoordinates (w - 4) (h - 4) (start + 2) (nbSquares - 1)
              (acc2
              @ [
                  (start, start);
                  (start, start + (w / 2));
      ...
      let rec createRow (j : int) (acc2 : square list) (nbNodes : int) : square list =
          if j > width
          then acc2
          else if List.mem (i, j) squarecoord
          then createRow (j + 1) (acc2 @ [Empty]) (nbNodes + 1)
          else if ((nbSquares - 1) * 2 < i && i < width - ((nbSquares - 1) * 2))
                  && (nbSquares - 1) * 2 < j
                  && j < width - ((nbSquares - 1) * 2)
          then createRow (j + 1) (acc2 @ [Wall]) nbNodes
          else if diagonal && i = j
          then createRow (j + 1) (acc2 @ [Path DL]) nbNodes
          else if diagonal && i = width - j
          then createRow (j + 1) (acc2 @ [Path DR]) nbNodes
          else if (List.mem (i, j - 1) squarecoord || getSquareRow acc2 (j - 1) = Some (Path H))
                  && (nbNodes != 3 || i = width / 2)
          then createRow (j + 1) (acc2 @ [Path H]) nbNodes
          else if (List.mem (i - 1, j) squarecoord || getSquare acc (i - 1, j) = Some (Path V))
                  && test3SquaresRow acc width height (i - 1) j 0 nbSquares
          then
            createRow (j + 1) (acc2 @ [Path V]) nbNodes
...
let initBoard2 (width : int) (height : int) (nbSquares : int) (diagonal : bool) : board =
    if match (width, height, nbSquares) with
...
let moveToDirection (game : gameUpdate) ((i, j) : coordinates) (d : directionDeplacement) (color : color) : gameUpdate =
    let rec goTo (game : gameUpdate) ((x, y) : coordinates) (d : directionDeplacement) (color : color) : gameUpdate =
        ...
        let case = getSquare game.board (cooBis d) in
        if case = Some (pathToHaveFromDirection d)
        then goTo game (cooBis d) d color
        else if case = Some Empty
        then moveToCoordinates game (i, j) (cooBis d) color
        else notUpdatedGame game
    in
    if getSquare game.board (coordinatesFromDirections d (i, j)) = Some (pathToHaveFromDirection d)
    then goTo game (i, j) d color
    else notUpdatedGame game

let possibleMoves (game : gameUpdate) ((i, j) : coordinates) (player : color) (diagonal : bool) :
    directionDeplacement list =
